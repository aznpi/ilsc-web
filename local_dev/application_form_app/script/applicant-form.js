...
const printApplicationSummary = function () {
    ...
    accommLink = schoolSelected == 'ELS' ? 'https://assets.contentstack.io/v3/assets/blte6ca0397941e65d8/blt4eddec62c8d065d4/6298de59c040564c4585393f/ELS_Cancellation_Policy.pdf' : 'https://www.accommodations.ilsc.com/accommodation-booking-policy';
    contractLink = countrySelected == 'Australia' ? 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-australia-terms-conditions.pdf' : countrySelected == 'Canada' ? 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-canada-terms-conditions.pdf' : countrySelected == 'India' ? 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-india-terms-conditions.pdf' : 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-canada-terms-conditions.pdf';
...
const resetSchoolProgram = function () {
    ...
    $('.week-input').empty();
    $('input[name=program_weeks_primary-input]').attr('disabled', true).val('');
...
const resetProgramFormField = function (elementStatus, formElement, emptyElement) {
    ...
    if (formElement) {
        ...
        $('input,select', formElement).attr('disabled', true);
...
const printWeeksOption = function (selectedProgramArray, element) {
    ...
    if (dataObject.length > 0) {
        ...
        $('.study-program-selection .input-container.schedule-input.' + element + ' .week-input').html(labelHtml + optionHtml);
    }
};
...
const printScheduleOption = function (selectedProgramArray, element) {
    let dataObject = selectedProgramArray,
    ...
    if (dataObject.length > 0) {
        ...
        for (let i = 0; i < dataObject.length; i++) {
            ...
            optionHtml += '<div class="form-check"><input class="form-check-input ' + element + '" value="' + dataObject[i].program_schedule + '" type="radio" name="program_schedule_' + element + '" data-weeks="' + dataObject[i].program_length + '" data-parent-id="' + dataObject[i].parent_program_id + '" id="' + dataObject[i].program_id + '" data-lasalle-eligible="' + lasalleVar + '" ' + singleOptionChek + ' required><label class="form-check-label" for="' + dataObject[i].program_id + '">' + dataObject[i].program_schedule + '</label></div>';
        }
        $('.study-program-selection .input-container.schedule-input.' + element + ' .time-input').html(labelHtml + optionHtml);
        $('.study-program-selection.study-box-container .input-container.schedule-input.' + element).removeClass('study-hide').addClass('study-show');
        printWeeksOption(dataObject, element);
        if (dataObject.length == 1) {
            $('input[name=program_weeks_' + element).val(dataObject[0].program_length).attr('disabled', false);
            printStartDates(dataObject[0].program_id, element, selectedMinAge, selectedMaxAge);

        }
    } else {
        $('.study-program-selection .input-container.schedule-input .time-input,.study-program-selection .input-container.schedule-input.' + element + ' .week-input').empty();
        printStartDates(selected, element, selectedMinAge, selectedMaxAge);
    }
};
const printProgramOption = function (selectedProgram, element, minAge, maxAge) {
        ...
        success: function (data) {
            ...
            if (programGroupedArray.length > 0) {
                for (let i = 0; i < programGroupedArray.length; i++) {
                    optionProgramHtml += '<div class="form-check"><input class="form-check-input ' + element + '" value="' + programGroupedArray[i].schedule_name + '-' + programGroupedArray[i].item_name + '" type="radio" name="program_option_' + element + '"  data-parent-id="' + programGroupedArray[i].group_id + '" id="' + programGroupedArray[i].program_id + '" ' + singleOptionChek + ' required><label class="form-check-label" for="' + programGroupedArray[i].program_id + '">' + programGroupedArray[i].schedule_name + '-' + programGroupedArray[i].item_name + '</label></div>';
                }
                $('.study-program-selection .input-container.program-option-input.' + element + ' .program-schedule-input').html(labelHtml + optionProgramHtml);
                $('.study-program-selection.study-box-container .input-container.program-option-input.' + element).removeClass('study-hide').addClass('study-show');
...
const printLaSalleOption = function (element) {

    isLasalleOptionHtml = "<label>Are you also applying for LaSalle partnership program?</label><div class='form-check'><input class='form-check-input " + element + "' value='No' type='radio' name='option_lasalle_parthership_" + element + "' id='no-'" + element + "'><label class='form-check-label' for='no-" + element + "'>No</label></div><div class='form-check'><input class='form-check-input " + element + "' value='Yes' type='radio' name='option_lasalle_parthership_" + element + "' id='yes-'" + element + "'><label class='form-check-label' for='yes-" + element + "'>Yes</label></div>";

    birthOptionHtml = "<div class='partner-info study-hide'><div class='form-group'><label for='partner-birth-city-" + element + "'>Enter City of Birth<sup>*</sup></label><input class='form-control' id='partner-birth-city-" + element + "' type='text' name='student_city_birth_" + element + "' disabled required></div><div class='form-group'><label for='partner-birth-state-" + element + "'>Enter Province/State of Birth<sup>*</sup></label><input class='form-control' type='text' id='partner-birth-state-" + element + "' name='student_prov_state_birth_" + element + "' disabled required></div></div>";

    $('.study-program-selection .partner-input.' + element + ' .partner-option-input').html(isLasalleOptionHtml + birthOptionHtml);
    $('.study-program-selection .partner-input.' + element).removeClass('study-hide').addClass('study-show');
};

const printStartDates = function (selectedProgram, element, minAge, maxAge) {
    let selectedCampus = $('input[name=program_campus]:checked').attr('id'),
        selectedParentSchool = $('input[name=program_school]:checked').attr('data-school'),
        selectedCountry = $('input[name=program_country]').val(),
    ...
    if (element == 'alternate-input') {
        tableId = startDateTableGC;
        primaryStartDate = $('select[name=program_startdate_primary-input] option:selected').attr('data-value');
        ...
        epochPrimaryStartDate = parseInt(primaryStartDate) + (1000 * 60 * 60 * 24 * primaryDuration);
    }

    if (typeof tableId !== 'undefined') {

        dateQuery = element == 'alternate-input' ? '&start_dates__gt=' + epochPrimaryStartDate : '';
        ...
        $('.study-program-start-date-dropdown .' + element + ' select.form-control').html("<option disabled selected value=''>No Start Dates Available</option>");
        $.get(api_url).done(function (data) {
            let dataObject = data.objects;
            ...
            if (dataObject.length > 0) {
                ...
                for (let i = 0; i < dataObject.length; i++) {
                    ...
                    if (currentDate.getTime() <= epochVal) {
                        ...
                        if (dateBirthTime <= minDob) {
                            ...
                            optionHtml = optionHtml + "<option " + optionHide + " value='" + dateInt + "' data-value='" + dataObject[i].values[3] + "' data-week-available='" + weekAvailableArray.toString() + "'>" + dateTxt + "</option>";
                ...
                ageQualifyMessage = "<option disabled selected value=''>No Start Dates Available. Possible reason - There is an age requirement for this program.</option>";

                if (enableOption) {

                    $('.' + element + ' .study-program-start-date-dropdown select.startdate-dropdown').html(optionHtml);
                    $('.' + element + ' .study-program-start-date-dropdown select.startdate-dropdown').prop("disabled", false);
                    if ($('.' + element + ' .study-program-start-date-dropdown select.startdate-dropdown option:not(:disabled)').length == 0) {
                        $('.' + element + ' .study-program-start-date-dropdown select.form-control').prop("disabled", false).html(ageQualifyMessage);
                    }
                ...
                $('.' + element + ' .study-program-start-date-dropdown select.form-control').prop("disabled", false).html(foundNoneHtml);
            }
            $('.study-program-selection.study-box-container .input-container.date-input.' + element).removeClass('study-hide').addClass('study-show');
        }).fail(function (jqXHR, textStatus, error) {
            console.log('Something went wrong: ' + error);
            $('.study-program-start-date-dropdown .' + element + ' select.form-control').html("<option disabled selected value=''>Cannot find Start Dates. Try another Program.</option>");
        });
    }
}

const printProgramDuration = function (element) {
    let selectedDuration = $('select[name=program_startdate_primary-input] option:selected').attr('data-week-available'),
        durationArray = selectedDuration.split(',');
    className = $('.study-program-selection .program-option-duration-input.' + element + ' .duration-dropdown select'),
    ...
    if (element == 'primary-input') {
        ...
        if (durationArray[0] != '') {
            for (let i = 0; i < durationArray.length; i++) {
                weekText = durationArray[i] == '1' ? 'week' : 'weeks';
                optionHtml += '<option class="dropdown-item" data-item="' + durationArray[i] + ' ' + weekText + '" value="' + durationArray[i] + ' ' + weekText + '">' + durationArray[i] + ' ' + weekText + '</option>';
            }
        } else {
            for (let z = 1; z < 53; z++) {
                weekText = z == 1 ? 'week' : 'weeks';
                optionHtml += '<option class="dropdown-item" data-item="' + z + ' ' + weekText + '" value="' + z + ' ' + weekText + '">' + z + ' ' + weekText + '</a>';
            }
        }
        $('.program-option-duration-input.primary-input').removeClass('study-hide').addClass('study-show');
...
const printProgramDrop = function (selectedCampus, isCoop, element) {

    let selectedSchool = $('input[name=program_school]:checked').attr('id'),
        selectedParentSchool = $('input[name=program_school]:checked').attr('data-school'),
        dropContainerEl = '.study-program-selection.study-box-container .input-container.program-input.' + element,
        printDropDownArray = [];
    programArray = [],
        isProla = false;
        ...
        $.get(api_url).done(function (data) {
            ...
            if (printDropDownArray.length > 0) {
                ...
                for (let i = 0; i < printDropDownArray.length; i++) {
                    ...
                    optionHtml = optionHtml + "<option data-id='" + printDropDownArray[i].program_id + "' value='" + optionValue + "' data-min-age='" + printDropDownArray[i].min_age + "' data-max-age='" + printDropDownArray[i].max_age + "'  data-prola='" + isProla + "' data-value='" + optionValue + "'>" + optionValue + "</option>";

                }
                $('.' + element + ' .study-program-dropdown select').html(optionHtml).attr('name', 'program_name_' + element).attr('id', 'program-name-' + element).addClass(element);
                $(dropContainerEl).removeClass('study-hide').addClass('study-show');
                $(dropContainerEl + ' select').attr('disabled', false);
            } else {
                $('.study-program-selection.study-box-container .input-container.program-input.' + element).removeClass('study-hide').addClass('study-show');
                $('.study-program-dropdown select.' + element + '').html('<option value="" selected disabled>No Programs available</option>');
            }
        });
    }
...
const printAdditionalInfo = function () {
    ...
    if (countrySelected == 'Australia') {
        ...
        inputHtml = selectedParentSchool == 'Greystone College' || isProla ? prolaHtml : '';
...
const coopAction = function (tEl, element) {
    let campusSelected = $('input[name=program_campus]:checked').val(),
        selected = $(tEl).val();
    resetProgramFormField(element, '.' + element + '.schedule-input,.' + element + '.date-input');
...
const programAction = function (tEl, element, pair) {
    ...
    resetProgramFormField(element, '.' + element + '.program-option-input,.' + element + '.schedule-input,.' + element + '.date-input,.' + element + '.program-option-duration-input', '.' + element + '.week-input');
...
const resetLaSalleOption = function () {
    $('.partner-input.' + element).removeClass('study-show').addClass('study-hide');
    $('.partner-input.' + element + ' input').attr('disabled', true);
};
const scheduleAction = function (tEl, element) {
    ...
    $('input[name=program_weeks_' + element).val(selectedWeeks).attr('disabled', false);
...
const startDateAction = function (element) {
...
const durationAction = function (element) {
        ...
        if ($('input[name=is_alternate_apply]:checked').val() == '1') {
            if (!$('.alternate-input .study-program-start-date-dropdown').is(':empty')) {
                let selectedId = $('input[name=program_schedule_alternate-input]:checked').attr('id'),
                    selectedMinAge = $('.study-program-dropdown select.' + element + ' option:selected').attr('data-min-age'),
                    selectedMaxAge = $('.study-program-dropdown select.' + element + ' option:selected').attr('data-max-age');
                printStartDates(selectedId, 'alternate-input', selectedMinAge, selectedMaxAge);
            }
...
$(document).on('click', '.primary-input input[name=program_is_coop_primary-input],.alternate-input input[name=program_is_coop_alternate-input]', function () {
...
$(document).on('click', '.primary-input input[name=program_option_primary-input]', function () {
    if ($(this).hasClass('alternate-input')) {
        element = 'alternate-input';
    } else {
        element = 'primary-input';
...
$(document).on('click', '.primary-input input[name=program_schedule_primary-input],.alternate-input input[name=program_schedule_alternate-input]', function () {
    if ($(this).hasClass('alternate-input')) {
        element = 'alternate-input';
    } else {
        element = 'primary-input';
...
$(document).on('click', '.primary-input input[name=option_lasalle_parthership_primary-input],.alternate-input input[name=option_lasalle_parthership_alternate-input]', function () {
    if ($(this).hasClass('alternate-input')) {
        element = 'alternate-input';
    } else {
        element = 'primary-input';
...
$(document).on('change', '.primary-input select[name=program_option_duration_primary-input]', function () {
