...
const printApplicationSummary = function(){
    let schoolSelected = $('input[name=program_school]:checked').attr('data-school'),
        ...
        for (let acc = 0; acc < accommodationFormArray.length; acc++) {
            ...
            if(accommodationFormArray[acc].name.includes('homestay')){
...
const printPersonForm = function(formArray,enabled,categoryName,className,dVal){
    ...
    for (let stForm in result) {
        ...
        let inputName,
            ...
            inputLabel = result[stForm].required == true? result[stForm].inputLabel+"<sup>*</sup>":result[stForm].inputLabel;
            ...
            success : function(data) {
                ...
                if(programGroupedArray.length >0){
                    for (let i = 0; i < programGroupedArray.length; i++){
                        optionProgramHtml += '<div class="form-check"><input class="form-check-input '+element+'" value="'+programGroupedArray[i].schedule_name+'-'+programGroupedArray[i].item_name+'" type="radio" name="program_option__'+element+'"  data-parent-id="'+programGroupedArray[i].group_id+'" id="'+programGroupedArray[i].program_id+'" '+singleOptionChek+' required><label class="form-check-label" for="'+programGroupedArray[i].program_id+'">'+programGroupedArray[i].schedule_name+'-'+programGroupedArray[i].item_name+'</label></div>'; 
                    }
...
const printStartDates = function(selectedProgram,element,minAge,maxAge){
    let selectedCampus = $('input[name=program_campus]:checked').attr('id'),
        ...
        selectedSchedule = $('input[name=program_option__primary-input]:checked').val(),
        dateBirth = new Date($('input[name=student_dob]').val()),
        dateBirthTime = dateBirth.getTime(),
        minorAge = 18;
...
const printAdditionalInfo = function(){
    let countrySelected = $('.study-information-form.dropdown-container button.country-select').attr('data-selected'),
        ...
        additionalNotesHtml = '<div class="form-group college-input study-show"><label for="additional-notes-input">Enter any additional notes regarding your program study</label><textarea name="study_additional_information_and_notes" id="additional-notes-input" cols="50" rows="10"></textarea><div class="valid-feedback"></div></div>';
        ...
        if(countrySelected == 'Australia'){
            ...
            additionalHtml = visaOptionUsHtml+visaFormRadioUsHtml+visaFormAddDependentButtonHtml+visaRadioUsHtml+visaTransferInputUsHtml+visaTransferAttendanceInputUsHtml+collegeRadioUsHtml+collegeAcceptRadioUsHtml+collegeInputUsHtml+additionalNotesHtml;
    ...
    if(element == 'primary-input'){
        if($('input[name=is_alternate_apply]:checked').val() == '1'){
            if(!$('.alternate-input .study-program-start-date-dropdown').is(':empty')){
                let selectedId = $('input[name=program_schedule__alternate-input]:checked').attr('id'),
                    selectedMinAge =  $('.study-program-dropdown select.'+element+' option:selected').attr('data-min-age'),
                    selectedMaxAge =  $('.study-program-dropdown select.'+element+' option:selected').attr('data-max-age');
...
$(document).on( 'click', '.primary-input input[name=program_option__primary-input]', function () {
    if($(this).hasClass('alternate-input')){
        element = 'alternate-input';
    }else{
        element = 'primary-input';
...
$(document).on( 'click', '.primary-input input[name=program_schedule__primary-input],.alternate-input input[name=program_schedule__alternate-input]', function () {
    if($(this).hasClass('alternate-input')){
        element = 'alternate-input';
    }else{
        element = 'primary-input';
