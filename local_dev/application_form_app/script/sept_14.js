...
const printApplicationSummary = function () {
    ...
    contractLink = countrySelected == 'Australia' ? 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-australia-terms-conditions.pdf' : countrySelected == 'Canada' ? schoolValSelected.includes('Juniors') ? 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/junior-student-contact-terms-conditions.pdf' :'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-canada-terms-conditions.pdf' : countrySelected == 'India' ? 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-india-terms-conditions.pdf' : 'https://www.ilsc.com/hubfs/pdf/applications-pricelists/student-contract-canada-terms-conditions.pdf';
    ...
    languageSchoolContractTxt = schoolValSelected.includes('Juniors') ? 'To finalize your application, you must acknowledge that you have read and agree to the student contract and must fill out other supplemental forms to help us administer the program to you. You can download this student contract and forms from the following link, sign them, and attach your signed copies here now using the file attachment link, or you may send them by email later. <a href="' + contractLink + '" target="_blank" download>Download</a>' : 'To finalize your application, you must acknowledge that you have read and agreed to the policies and procedures that apply to your chosen study country, and, if applicable, the Accommodation Policy. You can download the policies and procedures from the following link, sign it, and attach your signed copy here now using the file attachment link, or you may send it by email later. <a href="' + contractLink + '" target="_blank" download>Download</a>';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (programGroupedArray.length > 0) {
                ...
                $('#study-program-'+comboVal+'.study-program-selection .input-container.program-option-input.' + element + ' .program-schedule-input').html(labelHtml + optionProgramHtml);
...
const printStartDates = function (selectedProgram, element, minAge, maxAge, inputName) {
    ...
    if (typeof tableId !== 'undefined') {
        ...
        dateQuery = epochPrimaryStartDate != '' ? '&start_dates__gt=' + epochPrimaryStartDate : '';
