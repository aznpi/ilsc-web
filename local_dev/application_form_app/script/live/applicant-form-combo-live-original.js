...
const printStartDates = function (selectedProgram, element, minAge, maxAge, inputName) {
    ...
    let comboVal = returnComboVal(inputName),
        ...
        sessionStartDateMessage = '<sup>*</sup>Session Start Date'


    tableId = selectedParentSchool == 'Language School' ?  startDateTableLS : selectedParentSchool == 'Greystone College' ? startDateTableGC : selectedParentSchool == 'ELS' ? startDateTableELS : startDateTableLS;
    sessionVar = selectedParentSchool == 'Language School' | selectedParentSchool == 'ELS' ? 6 : selectedParentSchool == 'Greystone College' ? 5 : 6;
        ...
        $.get(api_url).done(function (data) {
            ...
            if (dataObject.length > 0) {
                ...
                for (let i = 0; i < dataObject.length; i++) {
                    ...
                    isSessionDate = dataObject[i].values[sessionVar] ? '<sup>*</sup>' : '';
                    hasSessionDate.push(dataObject[i].values[sessionVar]);
                    ...
                    if (currentDate.getTime() <= epochVal) {
                        ...
                        if (dateBirthTime <= minDob && dateBirthTime >= maxDob) {
                            ...
                            optionHtml = optionHtml + "<option " + optionHide + " value='" + dateInt + "' data-value='" + dataObject[i].values[3] + "' data-week-available='" + weekAvailableArray.toString() + "' "+selectedOption+">" +isSessionDate+ dateTxt + "</option>";
        ...
        $.get(api_url).done(function (data) {
            ...
            if (printDropDownArray.length > 0) {
                ...
                for (let i = 0; i < printDropDownArray.length; i++) {
                    ...
                    optionHtml = optionHtml + "<option data-id='" + printDropDownArray[i].program_id + "' value='" + optionValue + "' data-min-age='" + printDropDownArray[i].min_age + "' data-max-age='" + printDropDownArray[i].max_age + "' data-season='"+printDropDownArray[i].season+"' data-category='"+category+"' data-prola='" + isProla + "' data-value='" + optionValue + "' data-accommodation-enable='" + printDropDownArray[i].is_accommodation_enable + "'>" + optionValue + "</option>";
...
const printAdditionalInfo = function () {
    ...
    insideCountryHtml = '<label>Are you currently inside Canada?<sup>*</sup><span class="footnote">If yes, please submit your Study Permit or your Canadian visa (TRV or eTA) with your flight details that shows your arrival date in Canada. This information it’s required to process your registration.</span></label><div class="form-check"><input class="form-check-input" value="No" type="radio" name="additional_inside_canada" id="inside-question-no" checked required><label class="form-check-label" for="inside-question-no">No</label></div><div class="form-check"><input class="form-check-input" value="Yes" type="radio" name="additional_inside_canada" id="inside-question-yes" required><label class="form-check-label" for="inside-question-yes">Yes</label></div>';
    insidePermitFileHtml = '<div class="form-group inside-file-input study-hide"><label for="inside-permit-file">(Upload) Study Permit or your Canadian visa (TRV or eTA) (jpeg,png,pdf)</label><input type="file" name="program_inside_permit_file_upload" id="inside-permit-file" accept="jpeg,jpg,pdf,png" data-bind="study_permit_or_canadian_visa" disabled><input type="hidden" name="program_inside_permit_file_upload" value="" readonly disabled><div class="valid-feedback"></div></div>';
    insideFlightFileHtml = '<div class="form-group inside-file-input study-hide"><label for="inside-flight-file">(Upload) Flight details that shows your arrival date in Canada (jpeg,png,pdf)</label><input type="file" name="program_inside_flight_file_upload" id="inside-flight-file" accept="jpeg,jpg,pdf,png" data-bind="flight_details" disabled><input type="hidden" name="program_inside_flight_file_upload" value="" readonly disabled><div class="valid-feedback"></div></div>';
   ...
   additionalOnlineNotesHtml = '<div class="form-group college-input study-show"><label for="additional-notes-input">Enter any additional notes regarding your program study</label><textarea name="study_additional_online_information_and_notes" id="additional-notes-input" cols="50" rows="10"></textarea><div class="valid-feedback"></div></div>';
    ...
    if (countrySelected == 'Australia') {
        ...
        additionalHtml =  shirtHtml + jrFamilyHtml + insideCountryHtml + insidePermitFileHtml+ insideFlightFileHtml + inputHtml + additionalNotesHtml;
...
let programSelected = $('select[name*=program_name_] option:selected').val();
    ...
    if(enable){
        ...
        $(".accommodation-checkin-date-picker,.accommodation-checkout-date-picker").prop("disabled",true);
...
$(document).on('click', 'input[name=additional_visa_refusal]', function () {
    ...
    if ($(this).val() == 'Yes') {
        ...
        $('.gte-file-input input').attr('required', true).attr('disabled',false);
        printProgramHubspotFileForm();
    } else {
        $('.gte-file-input').removeClass('study-show').addClass('study-hide');
        $('.gte-file-input input').attr('required', false).attr('disabled',true);
    }
});
$(document).on('click', 'input[name=additional_inside_canada]', function () {

    if ($(this).val() == 'Yes') {
        $('.inside-file-input').removeClass('study-hide').addClass('study-show');
        $('.inside-file-input input').attr('required', true).attr('disabled',false);
        printProgramHubspotFileForm();
    } else {
        $('.inside-file-input').removeClass('study-show').addClass('study-hide');
        $('.inside-file-input input').attr('required', false).attr('disabled',true);
    }
});
$(document).on('click', 'input[name=additional_prola_taken]', function () {

    if ($(this).val() == 'Yes') {
        $('#prola-group').removeClass('disabled');
        $('input[name=additional_prola_number]').removeAttr('disabled');
    } else {
        $('#prola-group').addClass('disabled');
        $('input[name=additional_prola_number]').attr('disabled',true);
    }
});
