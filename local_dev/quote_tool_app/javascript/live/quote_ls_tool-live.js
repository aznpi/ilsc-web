...
const getProgramsList = function(){
  let isEdit = $('input[name=quote_edit]').val(),
      ...
      queryParam = '&campus__in='+campusName+'&show_quote_tool__eq=1&program_type__ne='+omitProgram+'&category__eq=Programs&orderBy=program_name',
    ...
    // jQuery cross domain ajax
      ...
      if(dataObject.length >0){
        for (let i = 0; i < dataObject.length; i++){
          ...
          radioHtml = "<div class='program-option'><input type='radio' autocomplete='off' class='program-radio' name='program_radio_selection' id='"+programId+"' data-description='"+programDescription+"' value='"+programName+"' "+selectEdit+"/><label for='"+programId+"'>"+programName+"</label></div>";
...
const getProgramsPriceListLowest = function(){
  let isEdit = $('input[name=quote_edit]').val(),
      ...
      queryParam = '&campus_location__in='+campusName+'&programs_available__in='+programName+'&is_program_price__eq=1&enable__eq=1&orderBy=-price_current',
      api_url = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam,
      programPriceArray = [];
  $('#online-msg').remove();
...
const getMarketLowestPriceProgramList = function(listArrayP){
  ...
  for( let y = 0; y < listArrayP.length; y++){
    ...
    queryPriceParam = '&price_list_code__in='+priceId+'&market_region__in='+marketPriceId+'&campus__in='+campusSelection;
...
const printDurationSelection = function(weekArray){
  ...
  if(weekArray.length > 0){
    for(let i = 0; i < weekArray.length; i++){
      htmlOption = htmlOption + '<a href="#" class="dropdown-item" data-item="'+weekArray[i].week+'" tabindex="0">'+weekArray[i].week+' Weeks</a>';
    }
  }else{
    for(let z = 1; z < 53; z++){
      htmlOption = htmlOption + '<a href="#" class="dropdown-item" data-item="'+z+'" tabindex="0">'+z+' Weeks</a>';      
    }
  }
...
const getStartDates = function(){
  let isEdit = $('input[name=quote_edit]').val(),
      ...
      queryParam = '&program__in='+programName+'&campus__in='+campus+'&start_dates__gt='+timestamp+'&orderBy=start_dates',
      api_url = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam;
  api_url = encodeURI(api_url);
...
const setProgramPriceDuration = function(listArray){
  ...
  for (let i = 0; i < listArray.length; i++){
    ...
    if(durationNum >= minVal && durationNum <= maxVal){
      ...
      xPrice = xConversion(xPrice);
      xStandardPrice = parseInt(listArray[i].priceStandard) * durationNum;
      xStandardPrice = xConversion(xStandardPrice);
...
const setProgramMarketPriceDuration = function(){
  let priceId = $('input[name=program_schedule_price_id_selection]').val(),
      ...
      queryParam = '&campus__in='+campus+'&price_list_code__in='+priceId+'&market_region__in='+regionId,
      api_url = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam;
  api_url = encodeURI(api_url);
...
const getOtherProgramFees = function(){
  let campus = $('input[name=campus_selection]').val(),
      programName = $('input[name=program_selection]').val(),
      tableId = schoolPriceListTable,
      queryParam = '&campus_location__in='+campus+'&category__eq=Other Program Fees&is_program_price__eq=0&programs_available__in='+programName+'&enable__eq=1&orderBy=-price_current',
      api_url = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam,
      feeArray = [];
  api_url = encodeURI(api_url);
