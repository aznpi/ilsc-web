...
const getProgramsList = function(){
  let isEdit = $('input[name=quote_edit]').val(),
      ...
      queryParam = '&campus__in='+campusName+'&show_quote_tool__eq=1&program_type__in='+programName+'&orderBy=program_name',
    ...
    $.get(api_url).done(function (data) {
      ...
      if(dataObject.length >0){
        for (let i = 0; i < dataObject.length; i++){
          ...
          programCategory = dataObject[i].values[12].name;
...
const getProgramsPriceList= function(){
  ...
  queryParam1 = '&campus_location__in='+campusName+'&category__eq='+programCategory+'&programs_available__in='+programId+'&enable__eq=1&orderBy=price_current',
      ...
      $.when(
        ...
        if(dataObject1.length > 0){
          programPriceArray = dataObject1.map(std => ({
            program_id: std.id,
            program_name: std.values[4],
            program_schedule: std.values[5].name,
            program_description: std.values[6],
            price_id: std.values[2],
            price:std.values[8],
            price_additional:std.values[31],
            price_additional_2:std.values[33],
            family_additional_enable:std.values[34]
          }));
    ...
    success : function(data) {
      ...
      for (let i = 0; i < dataObject.length; i++){
        ...
        programPriceArray.push({
          ...
          family_additional_enable:dataObject[i].values[34]
        });
...
const getStartDates = function(){
  ...
  if(programCategory.indexOf('Camps') !== -1){
    queryParam = '&program__in='+programId+'&campus__in='+campus+'&start_dates__gt='+timestamp+'&fixed_week_duration__in='+programDuration+'&orderBy=start_dates';
  }else{
