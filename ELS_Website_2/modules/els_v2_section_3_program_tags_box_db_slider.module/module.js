$(document).ready(function(){
    ...
    function resizePriceBoxes(){
      ...
      var resizeArray = ['h4','.description-container']
      var arrayLength = resizeArray.length;
      var maxHeight = [];
      
      for (var i = 0; i < arrayLength; i++) {
        maxHeight[i] = 0;
        $(".section-3-box-slider .box-3-tag-slider "+ resizeArray[i]).each(function () {
           if ($(this).height() > maxHeight[i]) { maxHeight[i] = $(this).height(); }
        });
        $(".section-3-box-slider .box-3-tag-slider "+ resizeArray[i]).height(maxHeight[i]);
      }
...
$('.section-3-box-program-slider .box-3-tag-slider').slick({
  infinite: false,
  arrows:true,
  slidesToShow: 3,
  slidesToScroll: 1,
