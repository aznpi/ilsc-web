...
const getProgramsList = function(){
  let isEdit = $('input[name=quote_edit]').val(),
      ...
      api_url_coop = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParamCoop;
  ...
  if ( $('#program-list-container').children().length == 0 ) {
    ...
    $.get(api_url).done(function (data) {
      let dataObject = data.objects.filter((elem,index) => data.objects.findIndex(obj => obj.values[29][0].id === elem.values[29][0].id) === index),
          radioHtml = '';

      if(dataObject.length >0){
        for (let i = 0; i < dataObject.length; i++){
          let selectEdit = '';
          programName = dataObject[i].values[3];
          programDescription = dataObject[i].values[22];
          programDescription = programDescription ? programDescription.replace(/['\u2019]/g,"&rsquo;") : '';
          programId = dataObject[i].values[29][0].id;
...
const getProgramsPriceList= function(){
  ...
  queryParam1 = '&country__in='+campusCountryName+'&parent_program_code__in='+programId+'&enable__eq=1&is_co_op__eq='+isCoop+'&orderBy=price_current',
    api_url_1 = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam1,
    queryParam2 = '&parent_program_code__in='+programId+'&market_region__in='+marketPriceId+'&is_co_op__eq='+isCoop,
    api_url_2  = 'https://api.hubapi.com/hubdb/api/v2/tables/'+marketTableId+'/rows?portalId='+portalId+queryParam2,
    programPriceArray = [],
    objects = [];
    ...
    $.when(
      ...
      if(dataObject2.length > 0){
        programPriceArray = dataObject1.map(std => ({
          program_id: std.values[2][0].id,
          program_name: std.values[3],
          program_schedule: std.values[10],
          standard_price: std.values[15],
          price_id: std.values[32],
          price:std.values[15],
          material_fee:std.values[19],
          campus: std.values[5],
          group_id: std.values[33][0].id,
          market_price: dataObject2.filter(mark  =>  mark.values[3][0].id === std.id)
        }));
        ...
        for (let i = 0; i < programPriceArray.length; i++){
          ...
          for (let x = 0; x < programPriceArray[i].campus.length; x++){
            ...
            if(programPriceArray[i].market_price.length > 0){
              marketPrice = programPriceArray[i].market_price[0].values[2];
            }else{
            ...
            campusPriceArray.push({
              ...
              campus: programPriceArray[i].campus[x].name,
      ...
      let programPriceGroupedByGroupCampus = groupBy(campusPriceArray, 'campus'),
...
const getStartDates = function(){
  ...
  let isEdit = $('input[name=quote_edit]').val(),
      ...
      queryParam = '&program__in='+programId+'&campus__in='+campus+'&start_dates__gt='+timestamp+'&orderBy=start_dates',
      api_url = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam;
  api_url = encodeURI(api_url);
...
const updateStartDateSelection = function(editNum){
  ...
  for (var i = 0; i < quoteJson.quote_array.length; i++) {
    if (i === objPosition) {     
      ...
      quoteJson.quote_array[i].program.program_id = priceId;
...
const getOtherProgramFees = function(){
  let campus = $('input[name=campus_selection]').val(),
      tableId = schoolPriceListTable,
      queryParam = '&campus__in='+campus+'&category__eq=Other Program Fees&enable__eq=1&orderBy=-price_current',
      api_url = 'https://api.hubapi.com/hubdb/api/v2/tables/'+tableId+'/rows?portalId='+portalId+queryParam;
  api_url = encodeURI(api_url);
  let feeArray = [];
  ...
  $.ajax({
    ...
    success : function(data) {
      ...
      for (let i = 0; i < dataObject.length; i++){
        ...
        feeArray.push({
          'feeId':  dataObject[i].values[32],
          'feeType' : dataObject[i].values[39],
          'feePrice' : dataObject[i].values[15],
          'feeDescription' : dataObject[i].values[3],
          'priceType' : dataObject[i].values[40].name,
        });
      }
