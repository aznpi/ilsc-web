...
if(!validArray.includes(false)){
    ...
    if(!formSubmit){
        setTimeout(submitStudentForm,100);
        setTimeout(sendProgramEnrolForm,500);
        setTimeout(sendAccommodationEnrolForm,1000); 
        setTimeout(sendPaymentEnrolForm,1500);
    }
    return true;
...
const printPaymentSummary = function(){
    let formObject = localStorage.getItem('quote-obj'),
        ...
        console.log('Printed Payment Summary');
...
const callPaymentStripe = function(){
    ...
    location.href =  payUrl +'?email='+ email +'&amount='+paymentAmount+'&code='+pId;
    //open the modal
    //$modal.modal('show');
